...
export default function DashboardPage() {
  ...
  const handleSend = async () => {
    ...
    setMessages((prev) => [...prev, { id: tempUserId, role: 'user', text: currentMessage }, { id: tempBotId, role: 'bot', text: '', status: 'PENDING' }]);
    ...
    try {
      ...
      if (genRes.success) {
        ...
        setMessages((prev) =>
          prev.map((m) =>
            m.id === tempBotId ? { ...m, status: 'FAILED', text: 'Error generating image: ' + (typeof genRes.error === 'string' ? genRes.error : JSON.stringify(genRes.error) || 'Unknown error') } : m,
          ),
        );
      }
    } catch (err) {
    ...
    <div className="flex-1 flex flex-col h-full relative">
            ...
            msg.role === 'bot' ? (
              <div key={msg.id} className="flex items-start gap-4">
                ...
                <div className="flex-1 mt-1">
                  ...
                  {msg.status === 'PENDING' ? (
                    ...
                    <p className="text-red-400 bg-red-950/30 px-4 py-3 rounded-2xl border border-red-900/50 w-fit text-sm">{msg.text || 'An error occurred.'}</p>
                  ) : msg.imageUrl ? (
                    ...
                    <p className="text-gray-300 leading-relaxed max-w-[85%] text-[15px]">{msg.text}</p>
                  )}
                </div>
              </div>
            ) : (
              <div key={msg.id} className="flex items-start gap-4 flex-row-reverse">
                ...
                <div className="flex-1 mt-1 flex flex-col items-end">
                  ...
                  <p className="text-gray-100 bg-[#2A2A2A] px-5 py-3 rounded-2xl rounded-tr-sm max-w-[85%] leading-relaxed text-left inline-block text-[15px] shadow-sm">{msg.text}</p>
                </div>
              </div>
            ),
          )}
      ...
      <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-[#0F0F0F] via-[#0F0F0F] to-transparent pt-12 pb-6 px-4 md:px-6 z-20">
        <div className="max-w-3xl mx-auto relative">
          ...
          <div className="text-center mt-3 text-[11px] text-zinc-500 tracking-wide font-medium">Nuvaria uses AI to imagine images. Some results may be unpredictable.</div>
        </div>
      </div>
    </div>
  );
